...
export const getAttendance = async (req, res) => {
  try {
    const { month, year, location, date, status } = req.query;
    if (!month || !year) {
    ...
    if (status === 'approved') {
      ...
      if (attendance) {
        ...
        if (oldStatus !== newStatus) {
          ...
          if (leaveAdjustment < 0 && employee.paidLeaves.available < Math.abs(leaveAdjustment)) {
...
export const markAttendance = async (req, res) => {
  try {
    ...
    if (!Array.isArray(attendance) || !attendance.length) {
      return res.status(400).json({ message: 'Attendance array is required and must not be empty' });
    }
    ...
    for (const entry of attendance) {
      if (!entry.employeeId || !entry.status || !entry.date) {
        return res.status(400).json({ message: 'Each attendance entry must have employeeId, status, and date' });
      }
...
export const bulkMarkAttendance = async (req, res) => {
  try {
    ...
    for (const dateStr of datesProcessed) {
      ...
      for (const emp of employees) {
        ...
        const entry = attendance.find(
          (a) => a.employeeId === empId && format(parseISO(a.date), 'yyyy-MM-dd') === dateStr
        );
        ...
        if (existingRecord) {
          ...
          if (oldStatus !== newStatus) {
            ...
            if (leaveAdjustment < 0 && emp.paidLeaves.available < Math.abs(leaveAdjustment)) {
              return res.status(400).json({
                message: `Employee ${emp.name} (${emp.employeeId}) has insufficient paid leaves for ${dateStr}`,
              });
            }
    ...
    console.error('Bulk mark attendance error:', { message: error.message, stack: error.stack, body: req.body });
...
export const undoMarkAttendance = async (req, res) => {
  try {
    ...
    console.error('Undo attendance error:', { message: error.message, stack: error.stack, body: req.body });
...
export const editAttendance = async (req, res) => {
  try {
    ...
    if (!status || !['present', 'absent', 'half-day', 'leave'].includes(status)) {
      return res.status(400).json({ message: 'Valid status is required (present, absent, half-day, leave)' });
    }
